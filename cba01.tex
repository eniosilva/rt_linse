...
Para a estimação dos parâmetros do vetor que caracteriza o modelo do trato vocal de banda alta UP, $\hat{\mathbf{q}}_{UP}$, uma vez determinada a classe, $\phi$, do \textit{frame} do segmento de fala NB em análise, as $K$ \textit{codewords} mais similares aos vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ do \textit{codebook} ${Y}^{\UP}_{{\CB}_{\phi}}$ são selecionadas via \textit{codebook dual} ${X}^{\NB}_{{\CB}_{\phi}}$, isto é,
\begin{equation}
\begin{array}{l}
...
\end{array}
\end{equation}
e os correspondentes vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ de banda alta são combinados linearmente com um peso $w$, calculado via distância euclidiana~\cite{unno05}, para cada \textit{codeword}. Assim,
\begin{equation}
