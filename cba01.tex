...
Tendo em vista que \textcolor{blue}{os serviços} comandados por fala são altamente dependentes do desempenho dos sistemas de ASR, as pesquisas sobre modelagem estatísticas de ASR continuam em franca evolução e se destacam como um tópico ativo em processamento digital de sinais \cite{Livescu2012}, \cite{Shaughnessy2013}, \cite{Bauer2014}.
...
\textcolor{red}{Em resumo}, um sistema típico de ASR é composto por duas etapas principais, o \textit{front-end} e o \textit{back-end}. A etapa de \textit{front-end} recebe o sinal de fala e \textcolor{blue}{efetua a} extração de vetores de observação contendo informações codificadas \textcolor{green}{das elocuções}. A etapa de \textit{back-end}, composta por um decodificador, dicionário fonético, modelo acústico e modelo de linguagem, é responsável pela decodificação dos vetores de observação e pelo processo de ``busca \textcolor{green}{e decodificação}'' \textcolor{green}{das elocuções} no sinal de fala \cite{HTKBook}, \cite{SpokenLanguage}.
...
\section{\textcolor{green}{Revisão} em reconhecimento automático de fala}
\label{backgroundASR}
\textcolor{green}{O objetivo de um sistema de ASR é interpretar adequadamente as informações contidas em um sinal de fala. Para tal, geralmente o ASR converte o sinal de fala em texto antes de executar qualquer ação. Esta seção apresenta uma breve introdução a \textcolor{red}{sistemas} de ASR.}
...
\label{eqAsr01}
\end{equation}
onde $p(X|\tau)$ representa a verossimilhança acústica entre a matriz de observação $X$ e as palavras da sentença $\tau$, \textcolor{red}{esta verossimilhança} é determinada por um modelo acústico previamente treinado. Visto que $p(X)$ não depende de $\tau$, (\ref{eqAsr01}) é equivalente a 
\begin{equation}
...
\section{\textcolor{green}{Revisão} em extensão artificial de largura de banda}
...
\begin{itemize}
\item Comprimento da janela\textcolor{red}{.\_}20 ms com sobreposição de 10 ms.
\item Coeficientes por segmento. \textcolor{red}{Energia, 12} coeficientes cepstrais da escala \textcolor{red}{mel, primeiras e segundas derivadas}.% No total, o vetor calculado para cada segmento tem 39 coeficientes.
...
Como apresentado na Tabela~\ref{resultado}, \textcolor{red}{o sistema} NSR proposto neste trabalho exibe um desempenho intermediário entre \textcolor{red}{os sistemas} NB PSTN e o sistema WB original, resultando em ganho relativo de aproximadamente $4,3$\% na WER quando comparado ao \textcolor{red}{sistema} NB convencional da PSTN.
