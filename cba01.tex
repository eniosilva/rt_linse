...
\begin{abstract}
 This work presents a framework for the implementation automatic speech recognition systems (ASR) for Brazilian Portuguese applied to the public switched telephone network (PSTN) using artificial bandwidth extension (ABWE). The state of the art shows that ASR systems operating with narrowband signals (NB) show lower performance systems that operate wideband signals (WB). In order to improve systems for PSTN, an acoustic model (MA) is developed based on synthetic WB signals estimated from NB signals applied to a ABWE system dedicated to phonetic classes of speech signals. Results of word error rate (WER) are evaluated and verified the effectiveness of framework implemented.
\end{abstract}
...
Sistemas de ASR aplicados a redes de telefonia podem ser implementados de acordo com três diferentes metodologias: reconhecimento de fala embarcado (\textit{embedded speech recognition} - ESR), que contêm as etapas de \textit{front-end} e \textit{back-end} embarcadas no aparelho telefônico; reconhecimento de fala distribuído (\textit{distributed speech recognition} - DSR), em que o \textit{front-end} é embarcado no aparelho telefônico e o \textit{back-end} é localizado em um servidor externo; e o reconhecimento de fala em rede (\textit{network speech recognition} - NSR), em que tanto o \textit{front-end} quanto o \textit{back-end} são localizados em um servidor remoto e o aparelho telefônico simplesmente envia o sinal de fala para ser processado nesse servidor \cite{Dmitry2006}.

Visando a obtenção de melhores taxas de reconhecimento, sistemas de ASR desenvolvidos para \textit{desktops} utilizam sinais de fala em banda larga (\textit{wideband} - WB) amostrados geralmente a taxas maiores do que 16 kHz. Sistemas aplicados à PSTN utilizam sinais de banda estreita (\textit{narowband} - NB) amostrados a 8 kHz e apresentam largura de banda de 300 a 3400 Hz. Devido às idiosincrasias do canal telefônico da PSTN \cite{EnioSBrT2013}, além da perda de naturalidade e inteligibilidade, esses sinais quando comparados com sinais de fala WB com largura de banda entre 50 e 7000 Hz, também apresentam perdas de desempenho em sistemas de ASR \cite{SpokenLanguage}.
...
\label{backgroundASR}
O objetivo de um sistema de ASR é converter um sinal de fala em texto. Esta seção apresenta uma breve introdução aos sistemas de ASR.
...
Na maioria das aplicações de ASR, a figura de mérito usada para avaliar um sistema de ASR é a taxa de erro de palavra (\textit{word error rate} - WER) e é definido como,
\begin{equation}
\label{WER}
...
Visando a obtenção de ótimos atributos discriminativos do sinal de fala, diversas alternativas para parametrizar as formas de onda desse sinal foram e continuam sendo desenvolidas. Dentre elas, a parametrização usando os coeficientes cepstrais da escala mel (\textit{mel frequency-cepstral coefficients-MFCCs}) mostra-se eficaz e é comumente usada no bloco de \textit{front-end} do ASR~\cite{SpokenLanguage}. Essa técnica de análise utiliza a escala mel, expressa por
%A escala de frequência em Hertz não reflete adequadamente a percepção auditiva humana, dessa forma, diversas alternativas para parametrizar as formas de onda do sinal de fala foram e continuam sendo desenvolidas~\cite{SpokenLanguage}. Dentre elas, a parametrização usando os coeficientes cepstrais da escala mel (\textit{mel frequency-cepstral coefficients-MFCCs}) mostra-se eficaz e é comumente usada como entrada para o bloco de \textit{back-end} do ASR. Essa técnica de análise utiliza a escala mel, expressa por
%Através de experimentos com diversos ouvintes, chegou-se na seguinte conversão de \textit{f} (em Hertz) para \textit{m} (em mel):
%A escala em Hertz não reflete bem a percepção auditiva humana. Para representá-la melhor, foi criada a escala mel \cite{SpokenLanguage}. Através de experimentos com diversos ouvintes, chegou-se na seguinte conversão de \textit{f} (em Hertz) para \textit{m} (em mel):
\begin{equation}
...
%$$m=M(f)=1125ln (1 + \frac{f}{700})$$
%$$f = M^{-1}(m)=700(e^{\frac{m}{1125}}-1)$$
Os coeficientes MFCCs são eficazes na extração de dados para o reconhecimento de fala. O procedimento de extração consiste em dividir o espectro do sinal em $B$ bandas, com frequências centrais igualmente espaçadas na escala mel.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%ESTOU AQUI NA REVISAO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Os coeficientes MFCCs são eficazes na extração de dados para o reconhecimento de fala. O procedimento de extração é ilustrado no diagrama da Figura TAL

O procedimento de extração consiste em dividir o espectro do sinal em $B$ bandas, com frequências centrais igualmente espaçadas na escala mel, Computar o Log e DCT, e selecionar os coeficientes resultantes para comporem os coeficientes mfccs. Dessa forma, as $B$ bandas de frequência correspondem ao ponto de partida para a construão dos descritores que compõem os vetores de observação MFCCs. Assim, os compenentes espectrais, ou seja, a resolução em frequecia é diretamente proporcional a qualidade dos descritores MFCCs. A Fig.~\ref{FigBancoFiltro} ilustra a distribuição dos filtros triangulares no dominio da frequencia.



a etapa inicial de construção dos descritores que irão compor os vetores de observação MFCC. No dominio do tempo as B bandas são expressas por filtros triangulres H(k)
Equação do Filtro
A Fig.~\ref{FigBancoFiltro} ilustra a distribuição de H(k) no dominio da frequencia.


Para isso, são aplicados filtros digitais triangulares $H(k)$, como mostra a Fig.~\ref{FigBancoFiltro}.

Por apresentarem uma maior faixa de frequência, sinais WB possuem uma quantidade maior de descritores na análise MFCC quando comparados à sinais NB. Portanto, os sinais NB provenientes da PSTN podem ser realçados através do sistema de ABWE que possibilitará a estimação e a sí1nteze dos componentes de frequência discriminativas capazes de preencher todos os filtros $H(k)$ dispostos na escala mel, exemplificado na Figura~\ref{FigBancoFiltro}.

\begin{figure}[h]
\center
\includegraphics[width=8cm]{figures/BancoFiltros.eps}
\caption{\label{FigBancoFiltro}Banco de filtros $H(k)$ e componentes espectrais.}
\end{figure}

\section{Background em extensão artificial de largura de banda}
\label{backgroundABWE}
O objetivo de um sistema de ABWE é realçar o sinal de fala NB aproximando-o de um sinal WB através da estimação dos componentes de frequências acima de $3.4$ kHz. A idéia básica de um sistema de ABWE é recriar artificialmente componentes de alta frequência, convertendo um sinal NB em um sinal WB, isto é, restaurando as propriedades acústicas pertinentes a WB. Essa técnica assume que ambos sinais, NB e WB, são gerados pelo mesmo modelo fonte-filtro de produção da fala~\cite{EnioSBrT2013}.

\subsection{Sistema de ABWE}
A Fig.~\ref{FigSistemaABWE} ilustra o diagrama de blocos do sistema de ABWE implementado em \cite{EnioSBrT2013} e aqui novamente adotado. O diagrama tem como base uma estrutura de três estágios de processamento, descritos como segue:
\begin{enumerate}
...
\label{FrameworkASRABWE}
No cenário de telefonia, sistemas NSR são aptos à utilização dos recusrsos computacionais de um servidor remoto, desta forma técnicas de processamento digital de sinais, tais como o realce da fala, podem ser melhor exploradas. Neste contexto, este artigo propõe a utilização de um \textit{framework} em que o sinal NB $S_{nb}$, fornecido pela PSTN, é realçado através de procedimentos de ABWE, gerando assim um sinal WB sintético $\hat{S}_{wb}$ na entrada do ASR, como ilustrado na Fig.~\ref{FigSistemaNSR}.
\begin{figure*}
  \centering
  %\includegraphics{figuras/Fig_1.eps}
%   \includegraphics[scale=0.98]{figuras/Fig_1.eps}
...
%%\begin{minipage}{0.5\linewidth}
%\center
%\includegraphics[width=8cm]{nsr02.eps}
%\caption{\label{FigSistemaNSR}Diagrama do ASR com ABWE em um sistema NSR.}
%%\end{minipage}
%\end{figure}

Dessa maneira, o sistema NSR será composto pelos blocos de ABWE, seguido dos blocos do ASR: \textit{front-end} e \textit{back-end}.
...
\end{equation}
onde o vetor $\mathbf{t}_n$ representa o envelope temporal contendo energias logarítmicas de $16$ subquadros ($1,25$ ms cada) \cite{Bernd2007} e o vetor $\mathbf{f}_{\LSF,n}$, componentes LSFs que caracterizam o envelope espectral. Tais parâmetros são estimados através de consulta a um conjunto de \textit{codebooks} selecionados de acordo com suas classes fonéticas~\cite{EnioSBrT2013}. 

Como ilustrado na Fig.~\ref{FigTreinamentoCB}, o processo de treinamento do sistema de ABWE consiste na construção do conjunto de \textit{Codebooks} correspodentes aos envelopes do trato vocal no processo de geração da fala.
...
O processo de treinamento aqui utilizado considera as técnicas de \textit{codebooks} duais de banda estreita $X^{\NB}_{\CB}$ e de banda alta $Y^{\UP}_{\CB}$~\cite{unno05}, criados a partir do agrupamento dos vetores de parâmetros  $x^{NB}$ e $y^{UP}$ em classes fonéticas dispostas de acordo com a Tabela~\ref{TebelaClasses}
...
Dessa maneira, um treinamento supervisionado é realizado e o agrupamento dos vetores de parâmetros  $x^{NB}$ e $y^{UP}$ torna-se mais discriminativo. Assim cada \textit{codebook} é associado a uma classe fonética específica $\phi$,
\begin{equation}
\begin{array}{r}
Y^{\UP}_{{\CB}_{\phi}}=E\{Y^{\UP} | \phi=Cl_i\}\\ % \, \in \, \Re^9 %\, \forall \, i=1 \ldots 9
X^{\NB}_{{\CB}_{\phi}}=E\{X^{\NB} | \phi=Cl_i\} 
\end{array}, \quad \in \, \Re^9
\end{equation}
...
Para a estimação das classes fonéticas, $\phi \in \{Cl_1, \ldots, Cl_9\}$, foi utilizado o algoritmo de árvore de decisão J48 disponível em~\cite{bookweka} e~\cite{algorithmJ48}. A cada \textit{frame} de 20 ms são extraídos vetores de observação contendo os seguintes parâmetros: os 10 primeiros coeficientes de auto-correlação, a taxa de cruzamento por zero, o índice de gradiente, a energia normalizada do frame, local Kurtosis e a centróide espectral, como proposto em ~\cite{PJaxThesis}. A partir dos vetores de observação, a árvore de decisão é consultada e a classe fonética correspondente é obtida. A abordagem de agrupamento de classes similares, vide Tabela~\ref{TebelaClasses}, aumenta a robustez quanto a erros de classificação, reduzindo eventuais imprecisões de singularidades acústicas devido as classes fonéticas estimadas, quando falhas, não estarem tão distantes de suas versões corretas.

Para a estimação dos parâmetros do vetor $\hat{\mathbf{q}}_{UP}$ que caracteriza o modelo do trato vocal de banda alta UP, uma vez determinada a classe, $\phi$, do quadro do segmento de fala NB em análise, as $K$ \textit{codewords} mais similares aos vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ do \textit{codebook} ${Y}^{\UP}_{{\CB}_{\phi}}$ são selecionadas via \textit{codebook dual} ${X}^{\NB}_{{\CB}_{\phi}}$, i.e. $\mathbf{Q}(n | Y^{\UP}_{\CB_{\phi}}) = \{\mathbf{q}^{\UP}_1(n|{X}^{\NB}_{{\CB}_{\phi}}),\mathbf{q}^{\UP}_2(n|{X}^{\NB}_{{\CB}_{\phi}}),...,\mathbf{q}^{\UP}_{K}(n|{X}^{\NB}_{{\CB}_{\phi}})\}$, e os correspondentes vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ de banda alta são combinados linearmente com um peso $w$ para cada \textit{codeword}. Assim,
\begin{equation}
...
%\end{equation}
onde $\mathbf{q}^{\UP}_{m}$ representa as \textit{codewords} constituídas pelos parâmetros $\mathbf{f}_{\LSF}, \mathbf{t}$ de banda alta. Assim,
\begin{equation}
\begin{array}{c}
...
O reconhecimento automático de fala é uma tecnologia dependente de dados e requer uma quantidade relativamente grande de dados rotulados para que seja possível a obtenção de taxas aceitáveis de precisão. O estado-da-arte em ASR baseado em HMM também exige um banco de dados de áudio (corpora), com vários locutores, e os MLs necessitam de milhões de sentenças para uma modelagem precisa da língua em questão. Desta forma, a disponibilidade de corpora é um fator primordial para o desenvolvimento de modelos acústicos e de linguagem. Um corpora típico possui arquivos de voz com as suas transcrições associados, grande quantidade de textos escritos e um dicionário fonético. Para atender tais necessidades, neste trabalho o desenvolvimento da etapa de ASR utiliza os corporas de fala disponibilizados em \cite{GrupoFalaBrasil} e \cite{YnogutiSBrT2008}.
...
\subsection{Contruão de Modelo Estatísticos para o ASR}
De acordo com a equação~\ref{eqAsr02}, $P(X|\tau)$ e $P(\tau)$, são determinados a partir dos modelos acústicos e de linguagem, respectivamente. A estimação de um bom modelo acústico é considerada a etapa mais desafiadora do projeto de um sistema ASR. Nesse contexto, os procedimentos do ABWE discutidos na seção anterior são responsáveis pelo realce na matriz de observação $X$, migrando de um espaço de dados NB para WB, assim os parâmetros MFCCs análisados pelo \textit{front-end} são melhores explorados. Nesse trabalho, o software HTK foi utilizado para construir modelos acústicos usando HMMs, de acordo com as etapas descritas em~\cite{HTKBook}. Para a cosntrução do modelo de linguagem, adotou-se a ferramenta MITLM~\cite{mitlm}.
...
Abaixo segue uma lista com detalhes sobre as configurações usadas:
%Abaixo está uma lista com detalhes sobre a configuração usada:

\begin{itemize}
\item Comprimento da janela: 25ms com sobreposição de 10ms.
...
\label{resultados}
Para análise de resultados, o corpora \textit{LapsBenchmark}~\cite{GrupoFalaBrasil} foi adotado e os resultados das WERs (definada na Equação~\ref{WER}), foram utilizados para avalização dos sistemas. Os resultados obtidos com os sistemas NB convencional da PSTN e o sistema WB foram utilizados como referência para a avaliação de desempenho do sistema NSR desenvolvido a partir da estratégia proposta neste artigo. A Tabela~\ref{resultado} apresenta os resultados obtidos a partir do sistema WB original, do sistema NB convencional e do sistema NSR onde os sinais WB são sintetizados através de ABWE.
...
De acordo a Tabela~\ref{resultado}, o sistema NSR proposto neste trabalho apresenta um desempenho intermediário entre os sistemas NB PSTN e o sistema WB original, resultando em ganho absoluto de $1,32$\% na WER.
