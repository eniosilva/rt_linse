...
\begin{abstract}
 Este trabalho apresenta \textcolor{black}{uma nova estratégia} para a implementação \textcolor{black}{de sistemas} de reconhecimento automático de fala (\textit{automatic speech recognition} - ASR) \textcolor{black}{aplicados} à rede pública de telefonia (\textit{public switched telephone network} - PSTN). O estado da arte reporta que sistemas de ASR que decodificam sinais de banda estreita (\textit{narrowband} - NB) apresentam desempenho inferior aos sistemas que operam com sinais de banda larga (\textit{wideband} - WB). Visando o aprimoramento de sistemas de ASR em PSTN, as etapas de extração de atributos do sinal da fala bem como a etapa de construção do modelo acústico (MA) são desenvolvidas \textcolor{black}{baseadas} em sinais sintéticos WB estimados a partir do realce de sinais NB usando extensão artificial de largura de banda (\textit{artificial bandwidrh extension} - ABWE). Resultados de taxa de erro de reconhecimento são avaliados e comprovam a eficácia \textcolor{black}{da estratégia} proposta.
\end{abstract}
...
\selectlanguage{brazil}
\section{Introdução}
Atualmente o mercado de telefonia dispõe de diversos serviços interativos, tais como os serviços presentes em sistemas automatizados de \textit{help desk}, portais de voz, gerenciamento de diálogos em unidades de resposta audível (URA) e outros tipos de atendimento via \textit{call centers} \cite{Oliveira2012}, como \textcolor{black}{apresentado} na Fig.~\ref{ExemploURA}.

%Tais serviços auxiliam o acesso de usuários da rede pública de telefonia (\textit{public switched telephone network} - PSTN) a informações via navegação em menus interativos. Esses menus podem ser acessados com a ajuda do teclado telefônico ou diretamente através de comandos de fala, em que o \textcolor{red}{assunto desejado} pelo usuário é \textcolor{red}{automaticamente identificado} usando sistemas de reconhecimento automático de fala (\textit{automatic speech recognition} - ASR) aplicados à PSTN \cite{Nelson2011}.
Tais serviços auxiliam o acesso de usuários da rede pública de telefonia (\textit{public switched telephone network} - PSTN) a informações via navegação em menus interativos. Esses menus podem ser acessados com a ajuda do teclado telefônico ou diretamente através de comandos de fala, \textcolor{black}{em que o usuário é atendido por um assistente virtual controlado por reconhecimento automático de fala (\textit{automatic speech recognition} - ASR) aplicado à PSTN} \cite{Nelson2011}.

%em que o usuário é atendido por um assistente virtual comandado por reconhecimento autoático de fala

Tendo em vista que \textcolor{black}{os serviços} comandados por fala são altamente dependentes do desempenho do ASR, as pesquisas sobre modelagem estatísticas desses sistemas continuam em franca evolução e se destacam como um tópico ativo em processamento digital de sinais \cite{Livescu2012}, \cite{Shaughnessy2013}, \cite{Bauer2014}.
...
\textcolor{black}{Resumidamente}, um sistema típico de ASR é composto por duas etapas principais, o \textit{front-end} e o \textit{back-end}. A etapa de \textit{front-end} recebe o sinal de entrada e \textcolor{black}{efetua a} extração de vetores de observação contendo informações codificadas \textcolor{black}{da fala}. A etapa de \textit{back-end}, composta por um decodificador, dicionário fonético, modelo acústico e modelo de linguagem, é responsável pela decodificação dos vetores de observação e pelo processo de ``busca'' \textcolor{black}{das informações linguísticas, e.g. fonemas e/ou palavras,} contidas no sinal de fala \cite{HTKBook}, \cite{SpokenLanguage}.
...
\textcolor{black}{Os sistemas} de ASR \textcolor{black}{aplicados} a redes de telefonia podem ser implementados de acordo com três diferentes metodologias: reconhecimento de fala embarcado (\textit{embedded speech recognition} - ESR), que contêm as etapas de \textit{front-end} e \textit{back-end} \textcolor{black}{integradas} no aparelho telefônico; reconhecimento de fala distribuído (\textit{distributed speech recognition} - DSR), em que o \textit{front-end} é \textcolor{black}{integrado} no aparelho telefônico e o \textit{back-end} é localizado em um servidor externo; e o reconhecimento de fala em rede (\textit{network speech recognition} - NSR), em que tanto o \textit{front-end} quanto o \textit{back-end} são localizados em um servidor remoto e o aparelho telefônico apenas envia o sinal de fala para ser processado nesse servidor \cite{Dmitry2006}.

Visando a obtenção de melhores taxas de reconhecimento, sistemas de ASR desenvolvidos para \textit{desktops} utilizam sinais de fala em banda larga (\textit{wideband} - WB) amostrados geralmente a taxas maiores do que 16 kHz. Entretanto, sistemas aplicados à PSTN utilizam sinais de banda estreita (\textit{narowband} - NB) \textcolor{black}{(apresentando largura de banda de 300 a 3400 Hz)} amostrados a 8 kHz e devido às \textcolor{black}{características inerentes ao} canal telefônico da PSTN \cite{EnioSBrT2013}, além da perda de naturalidade e inteligibilidade, esses sinais quando comparados com sinais de fala WB com largura de banda entre 50 e 7000 Hz também apresentam perdas de desempenho em sistemas de ASR \cite{SpokenLanguage}.
...
Neste artigo, \textcolor{black}{uma estratégia} para sistemas de ASR com metodologia NSR é proposta e o desempenho do ASR com ABWE é discutido. A eficácia \textcolor{black}{da estratégia implementada} é verificada através de avaliações objetivas da taxa de erro de palavras (\textit{word error rate} - WER).% e \textit{phone error rate} - PhER). 
...
Este artigo está organizado como segue. Nas Seções~\ref{backgroundASR} e~\ref{backgroundABWE}, são apresentadas visões gerais dos sistemas de ASR e ABWE. \textcolor{black}{Na Seção~\ref{FrameworkASRABWE}, é proposta a estratégia baseada na metodologia NSR e é descrito o desenvolvimento dos sistemas de ABWE e de ASR}. Finalmente, as Seções~\ref{resultados} e~\ref{conclusao} apresentam, respectivamente, os resultados obtidos, e conclusões e comentários finais deste trabalho de pesquisa.

%Este artigo está organizado como segue. Nas Seções~\ref{backgroundASR} e~\ref{backgroundABWE}, são apresentadas visões gerais dos sistemas de ASR e ABWE. \textcolor{green}{Na Seção~\ref{FrameworkASRABWE} são descritos os desenvolvimentos \textcolor{red}{dos sistemas} de ABWE e de ASR, e \textcolor{red}{os detalhes da contribuição deste artigo} são apresentados \textcolor{blue}{na estratégia proposta} baseada na metodologia NSR}. Finalmente, as Seções~\ref{resultados} e~\ref{conclusao} apresentam, respectivamente, os resultados obtidos, e conclusões e comentários finais deste trabalho de pesquisa.
%Este artigo está organizado como segue. Nas Seções~\ref{backgroundASR} e~\ref{backgroundABWE}, são apresentadas visões gerais dos sistemas de ASR e ABWE. Na Seção~\ref{FrameworkASRABWE}, é proposta \textcolor{blue}{uma estratégia} com base na metodologia NSR e o desenvolvimento \textcolor{red}{dos sistemas} de ABWE e de ASR é descrito, bem como \textcolor{red}{os detalhes da contribuição deste artigo}. Finalmente, as Seções~\ref{resultados} e~\ref{conclusao} apresentam, respectivamente, os resultados obtidos, e conclusões e comentários finais deste trabalho de pesquisa.
...
\section{\textcolor{black}{Revisão/Fundamentos} em reconhecimento automático de fala}
\label{backgroundASR}
\textcolor{black}{O objetivo de um sistema de ASR é estimar satisfatoriamente as informações contidas em um sinal de fala. Geralmente o procedimento intermediário desse sistema é a conversão do sinal de fala em texto. Esta seção apresenta uma breve introdução \textcolor{black}{a sistemas/aos fundamentos} de ASR.}

%O objetivo de um sistema de ASR \textcolor{red}{é converter um sinal de fala em texto}. Esta seção apresenta uma breve introdução a esses \textcolor{red}{sistemas}.
...
\subsection{\textcolor{black}{Arquitetura}}
\textcolor{black}{Um sistema típico de ASR é composto por cinco blocos principais}: \textit{front-end}, dicionário fonético, modelo acústico, modelo de linguagem e decodificador, sendo que os quatro últimos blocos compõem uma estrutura usualmente chamada \textit{back-end}, conforme indicado na Fig.~\ref{asr01}. 
...
\caption{\label{asr01} \textcolor{black}{Diagrama} de blocos de um sistema de ASR.}
%\end{minipage}
\end{figure}
...
\label{eqAsr01}
\end{equation}
onde $p(X|\tau)$ representa a verossimilhança acústica entre a matriz de observação $X$ e as palavras da sentença $\tau$, \textcolor{black}{essa verossimilhança} é determinada por um modelo acústico previamente treinado. Visto que $p(X)$ não depende de $\tau$, (\ref{eqAsr01}) é equivalente a 
\begin{equation}
...
Em resumo, após o treinamento dos modelos, o ASR, na fase de teste, usa o \textit{front-end} para converter o sinal de entrada em atributos discriminativos e usa o \textit{back-end} para estimar a sentença $\tau$ mais apropriada ao sinal de entrada $X$. Dessa forma, quanto \textcolor{black}{melhor} a qualidade do sinal de fala, maior o desempenho esperado do ASR. Nesse contexto, sistemas de ASR desenvolvidos para \textit{desktops} que utilizam sinais de fala WB, com frequência de amostragem geralmente maior do que 16 kHz, $f_s \ge 16 \textrm{ kHz}$, apresentam em média desempenho $5\%$ supeior a sistemas que utilizam sinais amostrados a 8 kHz, como é o caso dos sinais provenientes da PSTN~\cite{SpokenLanguage}.
%Em resumo, após o treinamento dos modelos, o ASR, na fase de teste, usa o \textit{front-end} para converter o sinal de entrada em parâmetros e o decodificador para procurar a melhor frase $\tau$. Dessa forma, quanto \textcolor{blue}{melhor} a qualidade do sinal de fala, maior o desempenho esperado do ASR. Nesse contexto, sistemas de ASR desenvolvidos para \textit{desktops} que utilizam sinais de fala WB, com frequência de amostragem geralmente maior do que 16 kHz, $f_s \ge 16 \textrm{ kHz}$, apresentam em média desempenho $5\%$ supeior a sistemas que utilizam sinais amostrados a 8 kHz, como é o caso dos sinais provenientes da PSTN~\cite{SpokenLanguage}.
...
O procedimento de extração de atributos consiste em dividir o espectro do sinal em $B$ bandas com frequências centrais igualmente espaçadas na escala mel. \textcolor{black}{Essas bandas de frequências são \textcolor{black}{distribuidas} através de bancos de filtros triangulares} e, para cada banda, são computatos os valores do logarítmo da energia \textcolor{black}{e a} transformada discreta do cosseno (\textit{discrete cosine transform-DCT}). Os valores resultantes compõem os coeficientes MFCCs, como ilustrado no diagrama da Fig.~\ref{FigMfccScheme}. 
\begin{figure}[h]
\center
...
%\textcolor{red}{Dessa forma, as $B$ bandas de frequência representam uma etapa de grande valia para a construção bem sucedida de descritores discriminativos que compõem os vetores de observação MFCCs}.
%\textcolor{green}{Dessa forma, a filtragem realizada nas $B$ bandas de frequência é fundamental para a construção de descritores discriminativos que compõem os vetores de observação MFCCs}. 
%\textcolor{green}{Dessa forma, a distribuição das $B$ bandas de frequência é fundamental para a construção de descritores discriminativos que compõem os vetores de observação MFCCs}. 
%\textcolor{green}{Dessa forma, o preenchimento adequado das $B$ bandas de frequência é fundamental para a construção de melhores descritores discriminativos}. Portanto, a largura de banda do espectro do sinal é diretamente proporcional a qualidade dos descritores MFCCs. Então, por apresentarem uma maior faixa de frequência, sinais WB possuem descritores de \textcolor{green}{maior resolução} na análise MFCC quando comparados a descritores obtidos a partir de sinais NB~\cite{Bauer2014}. 
\textcolor{black}{Dessa forma, a distribuição (adequada) dos componentes de frequência nas $B$ bandas é fundamental para a obtenção/o cálculo de ótimos atributos discriminativos}. Portanto, a largura de banda do espectro do sinal é diretamente proporcional a qualidade dos atributos MFCCs. Então, por apresentarem uma maior faixa de frequência, sinais WB possuem atributos de \textcolor{black}{maior resolução} na análise MFCC quando comparados a atributos obtidos a partir de sinais NB~\cite{Bauer2014}. 

\textcolor{black}{Entretanto, sinais NB podem ser realçados através de algorítmos de ABWE que possibilitam a estimação de novos componentes de frequência capazes de ampliar a largura de banda do espectro. Assim, os componentes de frequência, originais de NB e estimados de WB, podem ser redistribuídos eficientemente nos $B$ bancos de filtros triangulares (da análise MFCC)}. A Fig.~\ref{FigBancoFiltro} ilustra o processo de distribuição dos espectros NB e WB nos filtros triangulares correspondentes as $B$ bandas de frequência.
%\textcolor{green}{Entretanto, sinais NB podem ser realçados através de algorítmos de ABWE que possibilitam a estimação de novos componentes de frequência capazes de ampliar a largura de banda do espectro e preencher mais filtros triangulares distribuidos na escala mel}. A Fig.~\ref{FigBancoFiltro} ilustra o processo de distribuição dos espectros NB e WB nos filtros triangulares correspondentes as $B$ bandas de frequência.
...
\section{\textcolor{black}{Revisão/Fundamentos} em extensão artificial de largura de banda}
\label{backgroundABWE}
%O objetivo de um sistema de ABWE é realçar o sinal de fala NB, \textcolor{green}{melhorando sua qualidade e fazendo com que esse sinal se \textcolor{red}{assemelhe} à um sinal WB}. Isso é possível através da estimação dos componentes de frequências acima de $3.4$ kHz. Assim, a idéia básica de um sistema de ABWE é \textcolor{red}{recriar} artificialmente componentes de alta frequência, convertendo um sinal NB em um sinal WB, isto é, \textcolor{red}{restaurando} as propriedades acústicas pertinentes a WB~\cite{EnioSBrT2013},~\cite{Bauer2014}.
O objetivo de um sistema de ABWE é realçar o sinal de fala NB, \textcolor{black}{tornando-o mais agradável aos ouvintes e fazendo com que sua qualidade subjetiva se \textcolor{black}{assemelhe} à um sinal WB}. Isso é possível através da estimação dos componentes de frequências acima de $3.4$ kHz. Assim, a idéia básica de um sistema de ABWE é \textcolor{black}{sintetizar} artificialmente componentes de alta frequência, convertendo um sinal NB em um sinal WB, isto é, \textcolor{black}{estimando} as propriedades acústicas pertinentes a WB~\cite{EnioSBrT2013},~\cite{Bauer2014}.
...
\section{\textcolor{black}{\textit{Estratégia}} NSR usando ASR com ABWE}
\label{FrameworkASRABWE}
No cenário de telefonia, sistemas NSR são adequados à utilização dos recusrsos computacionais de um servidor remoto. Assim, \textcolor{black}{técnicas} de processamento digital de sinais, \textcolor{black}{tal como} o realce da fala, podem ser melhor exploradas. Nesse contexto, este artigo propõe  \textcolor{black}{uma estratégia} em que o sinal NB \textcolor{black}{$s_{NB}$}, fornecido pela PSTN, é realçado através de procedimentos de ABWE, gerando assim um sinal WB sintético \textcolor{black}{$\hat{s}_{WB}$} na entrada do ASR, como ilustrado na Fig.~\ref{FigSistemaNSR}.
%\begin{figure*}
%  \centering
%   \includegraphics[scale=0.25]{figures/nsr02.eps}
%  \centering
...
\begin{itemize}
\item Comprimento do \textit{fame} \textcolor{black}{igual} a $20$ ms com sobreposição de $10$ ms.
\item Coeficientes por \textit{fame} \textcolor{black}{ contendo valores de energia, 12} coeficientes cepstrais da escala \textcolor{black}{mel e, suas primeiras e segundas derivadas}.% No total, o vetor calculado para cada segmento tem 39 coeficientes.
\item MAs baseados em HMMs contínuas de 5 estados e topologia esquerda-direita, constituídas por modelos trifônicos \textit{cross-word} \textcolor{black}{desenvolvidos/computados} a partir de 38 monofones.
%\item Modelagem acústica efetuada utilizando HMMs contínuas com 5 estados e topologia esquerda-direita, constituída por modelos trifonicos \textit{cross-word} desenvolvidos a partir de 38 monofones.
...
%\item Modelagem da linguagem efetuada utilizando modelo tri-grama treinado com $1.534.980$ sentenças e técnica de suavização de \textit{Kneser-Ney}~\cite{mitlm}.
\end{itemize}
...
\label{resultados}
\textcolor{black}{Para que seja possível a obtenção de taxas aceitáveis de precisão}, o estado da arte de ASR, usando MAs baseados em HMMs, necessita de um banco de dados de áudio (\textit{corpora}) com grande variabilidade acústica, e os MLs necessitam de milhões de sentenças para uma modelagem precisa da língua em questão. Dessa forma, a disponibilidade de \textit{corpora} é um fator primordial para o desenvolvimento de modelos acústicos e de linguagem precisos. Um \textit{corpora} típico possui arquivos de voz com as suas transcrições associadas, grande quantidade de textos escritos e um dicionário fonético. Para atender tais requisitos, o desenvolvimento da etapa de ASR utiliza os \textit{corpora} de fala e texto disponibilizados em \cite{GrupoFalaBrasil} e \cite{YnogutiSBrT2008}.

Para análise de resultados, o corpora \textit{LapsBenchmark}~\cite{GrupoFalaBrasil} é adotado e os resultados das WERs, definada em (\ref{WER}), são utilizados na avalização \textcolor{black}{de três diferentes sistemas:} ASR com NB convencional da PSTN; ASR com WB; e NSR com ABWE e ASR. Os resultados obtidos com \textcolor{black}{os dois primeiros sistemas}, NB convencional da PSTN e  WB, são utilizados como referência para a avaliação de desempenho \textcolor{black}{da estratégia NSR proposta neste artigo}. 
...
%Para análise de resultados, o corpora \textit{LapsBenchmark}~\cite{GrupoFalaBrasil} é adotado e os resultados das WERs, definada em (\ref{WER}), são utilizados para avalização \textcolor{red}{dos sistemas}. Os resultados obtidos com \textcolor{red}{os sistemas} NB convencional da PSTN e \textcolor{red}{o sistema} WB são utilizados como referência para a avaliação de desempenho \textcolor{red}{do sistema} NSR desenvolvido a partir da estratégia proposta neste artigo. A Tabela~\ref{resultado} apresenta os resultados obtidos a partir \textcolor{red}{do sistema} WB original, \textcolor{red}{do sistema} NB convencional e \textcolor{red}{do sistema} NSR, no qual os sinais WB são sintetizados através de ABWE.
...
%Como apresentado na Tabela~\ref{resultado}, \textcolor{red}{a estratégia} NSR proposta neste trabalho apresenta um desempenho intermediário entre \textcolor{red}{os sistemas} NB PSTN e WB original, resultando em ganho relativo de aproximadamente $4,3$\% na WER quando comparado \textcolor{red}{(à  aplicação de) ao sistema} NB convencional da PSTN.
Como apresentado na Tabela~\ref{resultado}, \textcolor{black}{a estratégia} NSR proposta neste trabalho apresenta um desempenho intermediário entre \textcolor{black}{os sistemas} NB PSTN e WB original, resultando em ganho relativo de aproximadamente $4,3$\% na WER quando comparado \textcolor{black}{à  aplicação direta de sinais} NB convencionais na \textcolor{black}{(rede)} PSTN.
...
%Neste trabalho de pesquisa, uma estratégia de NSR utilizando um \textcolor{red}{sistema} de ABWE foi apresentada. Essa estratégia implementa o realce do sinal NB e resulta em um sinal estimado WB capaz de fornecer descritores cepstrais mais discriminativos ao ASR. O sinal WB estimado foi usado para a criação de modelos acústicos com maior riqueza espectral quando comparados às suas versões NB convencionais provenientes da PSTN. Resultados de avaliações objetivas também ratificam tais afirmações.
%Neste trabalho de pesquisa, uma estratégia de NSR utilizando ABWE foi apresentada. Essa estratégia implementa o realce do sinal NB e resulta em um sinal estimado WB capaz de fornecer atributos cepstrais mais discriminativos ao ASR. O sinal WB estimado foi usado para a criação de modelos acústicos com maior riqueza espectral quando comparados às suas versões NB convencionais provenientes da PSTN. Resultados de avaliações objetivas também ratificam tais afirmações.

%Tal resultado confirma a eficácia da estimação dos novos componentes de frequência do sinal sintetizado WB $\hat{s}_{WB}$, bem como a distribuição propícia(adequada/apropriada/conveniente/oportuna) desses componentes nas $B$ bandas de frequência.

Neste trabalho de pesquisa, uma estratégia de NSR utilizando ABWE foi apresentada. Essa estratégia implementa o realce do sinal NB \textcolor{black}{(de entrada)} e resulta em um sinal estimado WB capaz de fornecer \textcolor{black}{(artificialmente)} atributos cepstrais mais discriminativos ao ASR. \textcolor{black}{O sinal WB estimado (de entrada) proporcionou maior riqueza espectral ao treinamento de modelos acústicos, quando comparado ao treinamento usando suas versões NB convencionais provenientes da PSTN(, resultando em MAs mais precisos)}. Resultados de avaliações objetivas apresentados na Tabela~\ref{resultado} ratificam tais afirmações e confirmam a eficácia da estimação dos novos componentes de frequência do sinal sintetizado WB $\hat{s}_{WB}$, bem como a distribuição propícia\textcolor{black}{(adequada/apropriada/conveniente/oportuna)} desses componentes nas $B$ bandas de frequência da análise MFCC.
%Neste trabalho de pesquisa, uma estratégia de NSR utilizando ABWE foi apresentada. Essa estratégia implementa o realce do sinal NB \textcolor{red}{(de entrada)} e resulta em um sinal estimado WB capaz de fornecer \textcolor{red}{(artificialmente)} atributos cepstrais mais discriminativos ao ASR. \textcolor{red}{O sinal WB estimado foi usado para a criação de modelos acústicos com maior riqueza espectral quando comparados às suas versões NB convencionais provenientes da PSTN}. Resultados de avaliações objetivas apresentados na Tabela~\ref{resultado} ratificam tais afirmações e confirmam a eficácia da estimação dos novos componentes de frequência do sinal sintetizado WB $\hat{s}_{WB}$, bem como a distribuição propícia(adequada/apropriada/conveniente/oportuna) desses componentes nas $B$ bandas de frequência da análise MFCC.
