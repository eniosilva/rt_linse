...
Para a estimação dos parâmetros do vetor $\hat{\mathbf{q}}_{UP}$ que caracteriza o modelo do trato vocal de banda alta UP, uma vez determinada a classe, $\phi$, do quadro do segmento de fala NB em análise, as $K$ \textit{codewords} mais similares aos vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ do \textit{codebook} ${Y}^{\UP}_{{\CB}_{\phi}}$ são selecionadas via \textit{codebook dual} ${X}^{\NB}_{{\CB}_{\phi}}$, isto é,
\begin{equation}
\begin{array}{l}
...
\end{array}
\end{equation}
e os correspondentes vetores $\mathbf{f}_{\LSF,n}$ e $\mathbf{t}_n$ de banda alta são combinados linearmente com um peso $w$ para cada \textit{codeword}. Assim,
\begin{equation}
